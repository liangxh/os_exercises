...
(1)(spoc) 请参考lab3_result的代码，思考如何在lab3_results中实现clock算法，并给出你的概要设计方案，可4人一个小组，说明你的方案中clock算法与LRU算法上相比，潜在的性能差异性。并进一说明LRU算法在lab3实现的可能性评价（给出理由）。
 ...
 ```
需要复盖两个文件
1) swap_fifo.h -> swap_clock.h
使定义 exern struct swap_manager swap_mamger_clock;

2) swap_fifo.c ->swap_clock.h
同样使用链表记录物理页的信息, 但建立不是数据结构以保存访问信息
_clock_init_mm 内容同_fifo_init_mm
_clock_swap_out_victim 根据clock循环遍歴链表至找到合适的页
_clock_swap_in_page 添加新页并修改相应标记
